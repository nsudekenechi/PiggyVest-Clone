...
const BuildSavings = ({ item }) => {
    ...
    return (
        <Link href={`/${item.item.arrowText}`} className={`${item.index % 2 != 0 ? "pr-10" : ""}`}>
            <div className={`rounded-3xl shadow-sm relative  grid grid-cols-3 overflow-hidden duration-500`} style={{
                backgroundColor: !isHovered ? "white" : item.item.backgroundColor,
                color: !isHovered ? "black" : "white"
            }} onMouseOver={handleHover} onMouseOut={handleOut}>
                <div className="col-span-2 py-10 pl-10  flex flex-col justify-between h-[450px]" >
                    <div>
                        <h1 className=" text-4xl font-bold  mb-5" style={{
                            color: !isHovered ? item.item.textColor : "white"
                        }}>{item.item.title}</h1>
                        <p className="text-xl">{item.item.subtitle}</p>
                    </div>
                    ...
                    <Image src={item.item.img} className={`bottom-[0%] absolute duration-1000  ${!isHovered ? "right-[-100%] w-[0%]" : "w-[250px] right-5"}`} width={0} quality={100} alt={item.item.title} />
            </div>
        </Link>
    )
}
